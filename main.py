...
parser.add_argument(
    "--dataset", type=str, default="Augsburg", choices=dataset_names,help="Dataset to use."
)
parser.add_argument(
    "--model",
    type=str,
...
TRAIN_GT = args.train_set + DATASET +"/TRLabel.mat"
# Testing ground truth file
TEST_GT = args.test_set + DATASET + "/TSLabel.mat"
TEST_STRIDE = args.test_stride
BN_threshold = 2e-2
# set random seed
seed_torch(seed=args.seed)
os_path = you_should_save_here+"/"+MODEL+'/'+DATASET
if os.path.exists(os_path) is not True:
    os.makedirs(os_path)
writer = SummaryWriter(log_dir=os_path)
...
display_dataset(img1, gt, RGB_BANDS, LABEL_VALUES, palette, viz,writer=writer)
display_lidar_data(img2, viz,writer=writer)
...
for run in range(N_RUNS):
    ...
    display_predictions(convert_to_color(train_gt), viz, caption="Train ground truth",writer = writer)
    display_predictions(convert_to_color(test_gt), viz, caption="Test ground truth",writer = writer)
    # delete
    ...
    # Neural network
    model, optimizer, loss, hyperparams = get_model(MODEL,BN_threshold, **hyperparams)
    ...
    print(hyperparams)
    torch.save(hyperparams,'/tmp/output/Params.npy')

    # print("Network :")
    # with torch.no_grad():
    #     for input, input2,_,  _ in train_loader:
    #         break
    ...
    try:
        ...
        pass
    # model.load_state_dict(torch.load('/tmp/output/best_val_model.pkl'))
    ...
    show_results(run_results, viz, label_values=LABEL_VALUES,writer=writer)
    upload_output()

if N_RUNS > 1:
    show_results(results, viz, label_values=LABEL_VALUES, agregated=True,writer=writer)
