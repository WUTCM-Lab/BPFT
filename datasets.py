...
hmt_data_path = c2net_context.dataset_path+"/"+"ABH"
...
def get_dataset(dataset_name, target_folder="", datasets=DATASETS_CONFIG):
    ...
    tmpw = 9 // 2 + 1
    pad_width = [(tmpw,tmpw),(tmpw,tmpw),(0,0)]
    dataset = datasets[dataset_name]

    folder = target_folder + datasets[dataset_name].get("url")+ "/" +dataset_name + "/"
    ...
    if dataset_name == 'Houston':
        ...
        rgb_bands = (40,20,10)
        img1 = np.pad(img1,pad_width=pad_width,mode='constant', constant_values=0)
        
        img2 = open_file(folder + 'houston_lidar.mat')['houston_lidar'].astype(np.float32)
        img2 = np.expand_dims(img2, axis=2)
        img2 = np.pad(img2,pad_width=pad_width,mode='constant', constant_values=0)
        gt = open_file(folder + 'houston_gt.mat')['houston_gt']
        gt = np.pad(gt,pad_width=tmpw,mode='constant', constant_values=0)
        [m, n, l] = img1.shape
        for i in range(l):
            minimal = img1[:, :, i].min()
            maximal = img1[:, :, i].max()
        ...
        img1=open_file(folder + 'berlin_hsi.mat')['berlin_hsi'].astype(np.float32)
        rgb_bands=(40 , 20 , 10)  # To Do: fix
        img1 = np.pad(img1,pad_width=pad_width,mode='constant', constant_values=0)
        
        img2=open_file(folder + 'berlin_sar.mat')['berlin_sar'].astype(np.float32)
        img2 = np.pad(img2,pad_width=pad_width,mode='constant', constant_values=0)
        # img2 = np.expand_dims(img2, axis=2)  # (332, 485) --> (332, 485, 1)
        gt=open_file(folder + 'berlin_gt.mat')['berlin_gt']  # Here, the gt file is load for filtering NaN out and visualization.
        ...
        img1=open_file(folder + 'augsburg_hsi.mat')['augsburg_hsi'].astype(np.float32)
        rgb_bands=(40 , 20 , 10)  # To Do: fix
        img1 = np.pad(img1,pad_width=pad_width,mode='constant', constant_values=0)
        
        img2=open_file(folder + 'augsburg_sar.mat')['augsburg_sar'].astype(np.float32)
        img2 = np.pad(img2,pad_width=pad_width,mode='constant', constant_values=0)
        # img2 = np.expand_dims(img2, axis=2)  # (332, 485) --> (332, 485, 1)
        gt=open_file(folder + 'augsburg_gt.mat')['augsburg_gt']  # Here, the gt file is load for filtering NaN out and visualization.
...
class MultiModalX(torch.utils.data.Dataset):
    ...
    def __init__(self, data, data2, gt, **hyperparams):
        ...
        self.data = data
        self.data2 =data2
        self.data_pca = self.applyPCA(data,30)
        self.label = gt
    ...
    def __getitem__(self, i):
        ...
        data_pca = self.data_pca[x1:x2,y1:y2]
        label = self.label[x1:x2, y1:y2]

        if self.flip_augmentation and self.patch_size > 1:
            # Perform data augmentation (only on 2D patches)
            if np.random.random() > 0.5:
                data, data2,data_pca, label = self.flip(data, data2,data_pca, label)
            else:
                data, data2,data_pca, label = self.rotate(data, data2,data_pca, label)
        if self.radiation_augmentation and np.random.random() < 0.1:
            data = self.radiation_noise(data)
            data_pca = self.radiation_noise(data_pca)
        if self.mixture_augmentation and np.random.random() < 0.2:
            data = self.mixture_noise(data, label)
            data_pca = self.mixture_noise(data_pca,label)

        # Copy the data into numpy arrays (PyTorch doesn't like numpy views)
        data = np.asarray(np.copy(data).transpose((2, 0, 1)), dtype="float32")
        data2 = np.asarray(np.copy(data2).transpose((2, 0, 1)), dtype="float32")
        data_pca = np.asarray(np.copy(data_pca).transpose((2, 0, 1)), dtype="float32")
        ...
        return data, data2,data_pca, label
