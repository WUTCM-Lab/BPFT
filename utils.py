...
def display_predictions(pred, vis, gt=None, caption="",writer=None):
    if gt is None:
        if vis is not None:
            vis.images([np.transpose(pred, (2, 0, 1))],
                        opts={'caption': caption})
        if writer is not None:
            writer.add_image(tag=caption, img_tensor=np.transpose(pred, (2, 0, 1)))
    else:
        if vis is not None:
            vis.images([np.transpose(pred, (2, 0, 1)),
                        ...
                        opts={'caption': caption})
        if writer is not None:
            writer.add_image(tag="pred", img_tensor=np.transpose(pred, (2, 0, 1)))
            writer.add_image(tag="gt",img_tensor=np.transpose(gt, (2, 0, 1)))
...
def display_dataset(img, gt, bands, labels, palette, vis,writer = None):
    ...
    if vis is not None:
        vis.images([np.transpose(rgb, (2, 0, 1))],
                opts={'caption': caption})
    if writer is not None:
        writer.add_image(tag=caption,img_tensor=np.transpose(rgb,(2,0,1)))

def display_lidar_data(img, vis,writer=None):
    ...
    if writer is not None:
        writer.add_image(tag=caption,img_tensor=np.transpose(gray,(2,0,1)))
    ...
    for x in range(0, W - w + offset_w + 1, step):
        ...
        for y in range(0, H - h + offset_h + 1, step):
            if y + h > H:
                y = H - h
            if with_data:
                yield image1[x:x + w, y:y + h], image2[x:x + w, y:y + h],image_pca[x:x+w,y:y+h], x, y, w, h
            else:
                yield x, y, w, h
...
def show_results(results, vis, label_values=None, agregated=False,writer=None):
    text = ""
    ...
    print(text)
    file = open("/tmp/output/Result.txt","w")
    file.write(text)
    file.close()
...
def sample_gt(gt, train_size, mode='random'):
    ...
    if mode == 'random':
       ...
       for i in range(0,len(train_gt[0])):
            train_gt[train_indices[0][i]][train_indices[1][i]] = gt[train_indices[0][i]][train_indices[1][i]]
       for i in range(0,len(test_indices[0])):
            test_gt[test_indices[0][i]][test_indices[1][i]] = gt[test_indices[0][i]][test_indices[1][i]]
    elif mode == 'fixed':
